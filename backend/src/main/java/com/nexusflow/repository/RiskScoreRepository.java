...
public interface RiskScoreRepository extends JpaRepository<RiskScore, Long> {
     ...
     */
    @Query("SELECT rs FROM RiskScore rs WHERE rs.overallScore >= :threshold")
     ...
     */
    @Query("SELECT AVG(rs.overallScore) FROM RiskScore rs")
    Double getAverageRiskScore();

    /**
     * Finds top risky shipments.
     */
    default List<RiskScore> findTopRiskyShipments(int limit) {
